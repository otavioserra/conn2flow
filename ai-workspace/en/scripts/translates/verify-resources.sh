...
update_main_document() {
    ...
    if [ -f "$TRADUCAO_MAIN_FILE" ]; then
        # Update total files
        sed -i "s/- \*\*Total de Arquivos\*\*: [0-9]* (será atualizado)/- **Total Files**: $total_files/" "$TRADUCAO_MAIN_FILE"
...
main() {
    ...
    echo -e "${BLUE}📁 Resource list created at:${NC} $LISTA_RECURSOS_FILE"
    echo -e "${BLUE}📝 Main document updated:${NC} $TRADUCAO_MAIN_FILE"
}
