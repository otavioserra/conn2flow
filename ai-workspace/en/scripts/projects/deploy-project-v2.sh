...
upload_zip() {
    ...
    # Smart SSL verification: uses local CA with --ssl-no-revoke for Windows Schannel, or -k for .local, strict for production
    local ssl_opts=()
    local is_local=$(jq -r ".devProjects.\"$project_target\".local // false" "$ENV_FILE" 2>/dev/null)
    local ca_file="$HOME/.conn2flow-lab-ca.crt"
    if [ "$is_local" = "true" ] || [[ "$api_url" == *".local"* ]]; then
        ssl_opts+=(--ssl-no-revoke -k)
        if [ -f "$ca_file" ]; then
...
# Read git deploy config
GIT_DEPLOY=$(jq -r ".devProjects.\"$PROJECT_TARGET\".gitDeploy // false" "$ENV_FILE" 2>/dev/null)
GIT_DEPLOY_BASE_REF=$(jq -r ".devProjects.\"$PROJECT_TARGET\".gitDeployBaseRef // \"HEAD\"" "$ENV_FILE" 2>/dev/null)
...
create_zip_package() {
    if [ "$GIT_DEPLOY" = "true" ]; then
        ...
        if [ -n "$GIT_ROOT" ]; then
            ...
            RELATIVE_PATH="${PROJECT_PATH_WIN#$GIT_ROOT/}"
            if [ "$RELATIVE_PATH" = "$PROJECT_PATH_WIN" ]; then
                RELATIVE_PATH=""
            fi
            ...
            if [ -n "$RELATIVE_PATH" ]; then
                mapfile -t CHANGED_FILES < <(git diff --name-only --diff-filter=ACMR "$GIT_DEPLOY_BASE_REF" -- "$RELATIVE_PATH" 2>/dev/null || true)
            else
                mapfile -t CHANGED_FILES < <(git diff --name-only --diff-filter=ACMR "$GIT_DEPLOY_BASE_REF" 2>/dev/null || true)
            fi

            if [ ${#CHANGED_FILES[@]} -eq 0 ]; then
                log_warning "No changed files found compared to $GIT_DEPLOY_BASE_REF."
                log_warning "Falling back to packaging tracked files under project path."
                if [ -n "$RELATIVE_PATH" ]; then
        ...
        log_warning "gitDeploy enabled but no git repo root found for project path."
        log_warning "Packaging full project folder instead."
    fi
