...
## Removes all old tags matching gestor-v2* pattern locally and remotely
set +e
OLD_TAGS=$(git tag | grep "^gestor-v2")
if [ -n "$OLD_TAGS" ]; then
  echo "Removing all old tags matching gestor-v2* pattern: $OLD_TAGS"
