...
$.projeto_aplicar_scripts_after = function(params){
	...
	if(params){
		if(params['banner_href']){
			$.banner_rules(params['banner_href']);
		}
	}
};
...
$.banner_rules = function(href){
	...
	if(
		...
		$('#'+banner_principal).show();
		if(banner_count[banner_principal] > 1){
			$('#'+banner_principal+'-left').show();
			$('#'+banner_principal+'-right').show();
			//$('#'+banner_principal+'-pager').show();
		}
	} else {
		$('#'+banner_principal).hide();
		if(banner_count[banner_principal] > 1){
			$('#'+banner_principal+'-left').hide();
			$('#'+banner_principal+'-right').hide();
			//$('#'+banner_principal+'-pager').hide();
		}
	}
};
...
$(document).ready(function(){
	...
	banner_principal = 'banner-principal'; // Definição do ID do banner principal
	
	/* banner = 'banner-principal';
	if($('#'+banner).length){
		$('#'+banner)
			//.after('<div id="'+banner+'-pager" class="nav-cycle">') 
			.after('<div id="'+banner+'-left" class="link_hover nav-left">') 
			.after('<div id="'+banner+'-right" class="link_hover nav-right">')
			.cycle({
			fx: 'scrollHorz',
			timeout: 5000,
			speed: 600,
