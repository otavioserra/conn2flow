...
## SSH e rsync para a VM HestiaCP a partir do Git Bash (req-034)

Medido nesta máquina em 2026-09-02, contra `192.168.1.108`:

* **O `user@host:/caminho` do rsync ATRAVESSA o Git Bash intacto.** O `MSYS_NO_PATHCONV=1`
  necessário no `docker exec` não é necessário aqui — o argumento tem `:` e não
  começa com `/`, então o path conversion do MSYS não o toca. Confirme com
  `rsync --dry-run` antes de um sync grande, não depois.
* **Use sempre `-o BatchMode=yes`.** Sem isso, um problema de chave vira prompt de
  senha e o comando fica pendurado até o timeout do chamador, sem dizer o motivo.
  Some `-o ConnectTimeout=15` para transformar rede inacessível em erro rápido.
* **`exit 255` é do cliente SSH, não do comando remoto.** Traduzi-lo pela tabela de
  erros da aplicação produz diagnóstico no lugar errado (chave/host/rede
  reportados como falha de negócio).
* **A conta SSH não é dona do docroot.** `otavio` lê mas não escreve em
  `/home/admin/web/...`. Para o rsync funcionar: `--rsync-path="sudo rsync"`. E
  depois **devolva a posse** — arquivos novos ficam `root:root` e o pool PHP-FPM
  do tenant perde a leitura:
  `ssh ... "sudo chown -R admin:admin <caminho>"`.
* **Comando remoto passa pelo shell do servidor.** Mesmo montando a linha LOCAL como
  array (`proc_open` com `bypass_shell`), o `ssh` concatena os argumentos e o
  servidor os entrega ao shell. Todo valor que vem de formulário, banco ou
  `environment.json` precisa de `escapeshellarg()`/`printf %q` antes de entrar
  na linha remota.
* **Redirecionamento aninhado confunde o classificador de permissão.** Encadear
  `ssh ... 'sudo -u X env -i /usr/bin/ssh ...'` num único comando pode ser
  recusado. Prefira enviar um script com `scp` e executá-lo — além de passar, é
  reproduzível e revisável.

### `curl` autenticado contra o Gestor da VM

O Gestor faz uma sonda de suporte a cookie (`/_gestor-cookie-verify/<hash>/`)
antes de liberar rota autenticada. Um `curl -b jar` que só ENVIA cookies morre
em `302 → /signin`. É preciso **persistir** os cookies da sonda e seguir os
redirecionamentos:

```bash
curl -skL -b temp/agent-jar.txt -c temp/agent-jar.txt "https://conn2flow.local/host-manager/"
```

E os nomes de cookie carregam um sufixo derivado do host
(`_C2FCID_<8 hex de md5(basename($domainBase))>`). Se o sufixo do jar não bater
com o que a página emite, o cookie chega e é **ignorado em silêncio** — a tela
apenas redireciona para o login. Compare os dois antes de suspeitar do token:

```bash
grep -oE '_C2FCID_[0-9A-F]+' temp/agent-jar.txt
curl -sk https://conn2flow.local/ -c - | grep -oE '_C2F[A-Z]+_[0-9A-F]+'
```

### `open_basedir` do pool mente sobre existência de arquivo

`is_file()`/`is_executable()` obedecem `open_basedir`; `proc_open` não. Num pool
HestiaCP, checar `/usr/local/hestia/bin/v-*` com `is_file()` devolve `false` para
binário que existe. Antes de concluir "arquivo ausente" em log de PHP-FPM, leia
a cerca do pool.
