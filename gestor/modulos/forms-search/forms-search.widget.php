...
function forms_search_widget_options_html($field, $template = '') {
...
function forms_search_widget_render_results_cell($html, $form_id) {
	...
	if (!$found) {
		// req-141: este markup FICA em PHP de propósito. É o fallback para o template que não declara
		// o bloco de resultados — buscá-lo no template seria circular. As classes aqui são hooks de
		// JS (`forms-search-results`), não utilities do Tailwind: o compilador não precisa vê-las.
 ...
 */
function forms_search_widget_wrap_password($html, $template = '') {
	...
	return preg_replace_callback('/<input\b[^>]*>/i', function ($m) use ($modelo) {
		$aria = function_exists('gestor_variaveis')
			? (string)gestor_variaveis(Array('modulo' => 'forms-search', 'id' => 'password-toggle-aria'))
			: '';
...
function forms_search_widget_render_field($template, $field) {
	...
	$html = forms_search_widget_replace_var($html, 'item#options', forms_search_widget_options_html($field, $template));
	...
	if ($type === 'hidden') {
		...
		$html = forms_search_widget_wrap_password($html, $template);
	}
...
function forms_search_render($params) {
	...
	$registro = banco_select([
		'unico' => true,
		'tabela' => 'forms_search',
		// req-077 §2: mesma correção do widget de `forms` — `css_precompiled` carrega as utilities
		// Tailwind desde a pré-compilação por recurso.
		// A coluna existe em `templates`, `componentes`, `paginas` e
		// `layouts`, mas NÃO em `forms_search` — a migração da pré-compilação por recurso não
		...
		'campos' => banco_campo_existe('css_precompiled', 'forms_search')
			? ['id_forms_search', 'id', 'fields_schema', 'html', 'css', 'css_precompiled', 'css_compiled', 'html_extra_head']
			: ['id_forms_search', 'id', 'fields_schema', 'html', 'css', 'css_compiled', 'html_extra_head'],
		'extra' =>
