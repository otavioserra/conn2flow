...
$_GESTOR['modulo-id'] = 'forms-search';
$_GESTOR['modulo#'.$_GESTOR['modulo-id']] = json_decode(file_get_contents(__DIR__ . '/forms-search.json'), true);

// ===== Funções Auxiliares

function forms_search_normalize_array($array) {
	if (is_array($array)) {
		ksort($array);
		foreach ($array as $key => $value) {
			$array[$key] = forms_search_normalize_array($value);
		}
	}
	return $array;
}
...
function forms_search_schema_decode($fields_schema, $template_id = '') {
...
function forms_search_item_variables() {
	return [
		['id' => 'label'],
		['id' => 'name'],
		['id' => 'placeholder'],
...
function forms_search_convert_text_vars_to_storage($value) {
	global $_GESTOR;
...
function forms_search_convert_storage_vars_to_text($value) {
	global $_GESTOR;
...
function forms_search_template_options($selected_id = null, $has_custom_code = false) {
	global $_GESTOR;
...
function forms_search_prepare_editor_page($schema, $html = '', $css = '', $css_compiled = '', $html_extra_head = '', $modo = 'adicionarEditar') {
	global $_GESTOR;

	$modulo = $_GESTOR['modulo#'.$_GESTOR['modulo-id']];
	$has_custom_code = (!empty($html) || !empty($css));

	forms_search_template_options($schema['template_id'] ?? '', $has_custom_code);

	$_GESTOR['pagina'] .= '<script>var forms_search_initial_schema = '.json_encode($schema, JSON_UNESCAPED_UNICODE).';</script>';

	$params = [
		'modulo' => $modulo,
		'alvo' => 'forms-search',
		'alvos_modelos' => 'forms-search',
		'target_variables' => forms_search_item_variables(),
		'html' => $html,
		'html_extra_head' => $html_extra_head,
		// req-070 §1.3: o preview do editor de forms-search só precisa do script controlador de widget de forms-search.
		'widget_js_include' => ['forms-search' => true],
	];
	$params[$modo] = true;
...
function forms_search_insert_common_fields(&$campos, $id, $usuario) {
	global $_GESTOR;

	$campo_sem_aspas_simples = false;
	$fields_schema = $_REQUEST['fields_schema'] ?? '{}';
	$schema = forms_search_schema_decode($fields_schema, $_REQUEST['template_id'] ?? '');
	...
	$_REQUEST['css_compiled'] = forms_search_convert_text_vars_to_storage($_REQUEST['css_compiled'] ?? '');
	$_REQUEST['html_extra_head'] = forms_search_convert_text_vars_to_storage($_REQUEST['html_extra_head'] ?? '');
...
function forms_search_meta_dados($retorno_bd, $modulo) {
	$metaDados = [];
...
function forms_search_interface_finalizar($opcao, $id = null, $metaDados = [], $status_atual = '', $module = '') {
	global $_GESTOR;
...
function forms_search_adicionar() {
	...
	if (isset($_GESTOR['adicionar-banco'])) {
		...
		$campos = [];
		forms_search_insert_common_fields($campos, $id, $usuario);
...
function forms_search_editar() {
	...
	$camposBanco = ['id', 'id_forms_search', 'name', 'description', 'module', 'template_id', 'fields_schema', 'html', 'css', 'css_compiled', 'html_extra_head', 'status'];
	...
	if (isset($_GESTOR['atualizar-banco'])) {
		...
		if (isset($_REQUEST['fields_schema'])) {
			$request_schema = forms_search_schema_decode($_REQUEST['fields_schema'], $_REQUEST['template_id'] ?? '');
			$request_valor = json_encode($request_schema, JSON_UNESCAPED_UNICODE);
			$valor_request = forms_search_normalize_array(json_decode($request_valor, true));
			$valor_banco = forms_search_normalize_array(json_decode(banco_select_campos_antes('fields_schema'), true));
	...
	$css = $retorno_bd['css'] ?? '';
	$css_compiled = forms_search_convert_storage_vars_to_text($retorno_bd['css_compiled'] ?? '');
	$html_extra_head = forms_search_convert_storage_vars_to_text($retorno_bd['html_extra_head'] ?? '');
	$schema = forms_search_schema_decode($fields_schema, $template_id);
...
function forms_search_clonar() {
	...
	$modulo = $_GESTOR['modulo#'.$_GESTOR['modulo-id']];
	$id = $_GESTOR['modulo-registro-id'];
	$camposBanco = ['id', 'id_forms_search', 'description', 'module', 'template_id', 'fields_schema', 'html', 'css', 'css_compiled', 'html_extra_head', 'status'];
	...
	if (isset($_GESTOR['adicionar-banco'])) {
		...
		$campos = [];
		forms_search_insert_common_fields($campos, $new_id, $usuario);
		banco_insert_name($campos, $modulo['tabela']['nome']);
	...
	$module = $retorno_bd['module'] ?? '';
	$template_id = $retorno_bd['template_id'] ?? '';
	$schema = forms_search_schema_decode($retorno_bd['fields_schema'] ?? '', $template_id);
	$html = $retorno_bd['html'] ?? '';
	$css = $retorno_bd['css'] ?? '';
	$css_compiled = forms_search_convert_storage_vars_to_text($retorno_bd['css_compiled'] ?? '');
	$html_extra_head = forms_search_convert_storage_vars_to_text($retorno_bd['html_extra_head'] ?? '');
...
function forms_search_visualizar() {
	...
	$modulo = $_GESTOR['modulo#'.$_GESTOR['modulo-id']];
	$id = $_GESTOR['modulo-registro-id'];
	$camposBanco = ['id', 'id_forms_search', 'name', 'description', 'module', 'fields_schema', 'status'];
	...
	$fields_schema = json_encode(forms_search_schema_decode($retorno_bd['fields_schema'] ?? ''), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
	...
	$_GESTOR['interface']['visualizar']['finalizar'] = [
		...
		'id' => $id,
		'metaDados' => forms_search_meta_dados($retorno_bd, $modulo),
		'banco' => [
			'nome' => $modulo['tabela']['nome'],
			'id' => $modulo['tabela']['id'],
			'status' => $modulo['tabela']['status'],
...
function forms_search_interfaces_padroes() {
	global $_GESTOR;
...
function forms_search_ajax_template_load() {
	...
	$template = banco_select([
		...
		'extra' => "WHERE id='".banco_escape_field($template_id)."' AND target='forms-search' AND language='".$_GESTOR['linguagem-codigo']."' AND status='A'",
	]);
	...
	$template['html'] = forms_search_convert_storage_vars_to_text($template['html'] ?? '');
	$_GESTOR['ajax-json'] = [
		'status' => 'Ok',
		...
		'campos' => forms_search_item_variables(),
	];
}

function forms_search_ajax_widget_preview() {
	global $_GESTOR;
	...
	if (!function_exists('forms_search_widget_render_inline')) {
		require_once(__DIR__.'/forms-search.widget.php');
	}

	$rendered = forms_search_widget_render_inline([
		'html' => $html_input,
		'css' => $css_input,
		'form_id' => $form_id,
...
function forms_search_start() {
	...
	if ($_GESTOR['ajax']) {
		...
		forms_search_interfaces_padroes();
		interface_iniciar();

		switch ($_GESTOR['opcao']) {
			case 'visualizar': forms_search_visualizar(); break;
