...
$_GESTOR['modulo-id']							=	'admin-categorias';
$_GESTOR['modulo#'.$_GESTOR['modulo-id']] = json_decode(file_get_contents(__DIR__ . '/admin-categorias.json'), true);

// ===== Funções Auxiliares

function admin_categorias_categorias_pais_arvore($params = false){
	/**********
	...
	**********/
	...
	if($categoriaPaiBD){
		...
		if(existe($categoriaPaiBD[0][$modulo['tabela']['id_numerico_ref']])){
			$categoriasPais = admin_categorias_categorias_pais_arvore(Array(
...
function admin_categorias_categoria_localizacao($params = false){
	/**********
	...
	**********/
	...
	if(isset($id_categorias_pai) && isset($nome)){
		$arvoreCategoriasPais = admin_categorias_categorias_pais_arvore(Array(
		...
		if(count($arvoreCategoriasPais) > 0){
			...
			$urlRaiz = $_GESTOR['url-raiz'].'admin-categorias/';
			$urlComum = $_GESTOR['url-raiz'].'admin-categorias/editar/?id=#pai-id#';
...
function admin_categorias_adicionar_filho(){
	global $_GESTOR;
...
function admin_categorias_adicionar(){
	global $_GESTOR;
...
function admin_categorias_editar(){
	global $_GESTOR;
	...
	if($_GESTOR['banco-resultado']){
		...
		if(existe($id_categorias_pai)){
			$_GESTOR['pagina'] = modelo_var_troca($_GESTOR['pagina'],"#cont-categoria-pai#",admin_categorias_categoria_localizacao(Array(
				'id_categorias_pai' => $id_categorias_pai,
				'nome' => $nome,
			)));
		} else {
...
function admin_categorias_interfaces_padroes(){
	global $_GESTOR;
...
function admin_categorias_ajax_opcao(){
	global $_GESTOR;
...
function admin_categorias_start(){
	global $_GESTOR;
	...
	if($_GESTOR['ajax']){
		...
		switch($_GESTOR['ajax-opcao']){
			//case 'opcao': admin_categorias_ajax_opcao(); break;
		}
		...
		switch($_GESTOR['opcao']){
			case 'adicionar-filho': admin_categorias_adicionar_filho(); break;
			case 'adicionar': admin_categorias_adicionar(); break;
			case 'editar': admin_categorias_editar(); break;
		}
