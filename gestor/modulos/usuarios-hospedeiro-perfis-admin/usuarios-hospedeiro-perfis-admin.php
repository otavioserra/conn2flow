...
$_GESTOR['modulo-id']							=	'usuarios-hospedeiro-perfis-admin';
$_GESTOR['modulo#'.$_GESTOR['modulo-id']] = json_decode(file_get_contents(__DIR__ . '/usuarios-hospedeiro-perfis-admin.json'), true);
...
function hosts_usuarios_perfis_adicionar(){
	...
	if(isset($_GESTOR['adicionar-banco'])){
		...
		banco_update_executar($modulo['tabela']['nome'],"WHERE padrao IS NOT NULL AND id_hosts IS NULL");
...
function hosts_usuarios_perfis_editar(){
	global $_GESTOR;
	...
	if(isset($_GESTOR['atualizar-banco'])){
		...
		$hosts_usuarios_perfis_modulos = banco_select_name
		(
			banco_campos_virgulas(Array(
				'modulo',
			))
		...
		$hosts_usuarios_perfis_modulos_operacoes = banco_select_name
		(
			banco_campos_virgulas(Array(
				'operacao',
			))
		...
		for($i=1;$i<$numModulos;$i++){
			...
			if($_REQUEST['modulo-'.$i]){
				...
				if($encontrou){
					...
					foreach($hosts_usuarios_perfis_modulos as $upm){
		...
		for($i=1;$i<$numModulosOperacoes;$i++){
			if($_REQUEST['operacao-'.$i]){
				...
				if($encontrou){
					...
					if($hosts_usuarios_perfis_modulos_operacoes)
					foreach($hosts_usuarios_perfis_modulos_operacoes as $upmo){
...
function hosts_usuarios_perfis_start(){
	global $_GESTOR;
	...
	if($_GESTOR['ajax']){
		...
		switch($_GESTOR['ajax-opcao']){
			//case 'opcao': hosts_usuarios_perfis_ajax_opcao(); break;
		}
		
		interface_ajax_finalizar();
	} else {
		hosts_usuarios_perfis_interfaces_padroes();
