...
$_GESTOR['modulo-id']							=	'publisher-index';
$_GESTOR['modulo#'.$_GESTOR['modulo-id']] = json_decode(file_get_contents(__DIR__ . '/publisher-index.json'), true);

// ===== Funções Auxiliares

function publisher_index_normalize_array($array) {
    if (is_array($array)) {
        ksort($array);
        foreach ($array as $key => $value) {
            $array[$key] = publisher_index_normalize_array($value);
        }
    }
    return $array;
}
 ...
 * `publisher-index` (req-004 item 7).
 *
 * @param string $html
 * @return array Lista de [['id' => 'titulo'], ['id' => 'resumo'], ...] sem duplicatas.
 */
function publisher_index_extract_item_variables($html){
 ...
 */
function publisher_index_publisher_by_slug($publisher_id){
	global $_GESTOR;
...
function publisher_index_adicionar(){
	...
	publisher_index_publisher_options(null);

	// ===== Templates para seleção (dropdown template_id)

	publisher_index_template_options(null, false);
...
function publisher_index_editar(){
	...
	if(isset($_GESTOR['atualizar-banco'])){
		...
		if(isset($_REQUEST[$request_name])){
			...
			$valor_request = publisher_index_normalize_array(json_decode($request_formatado, true));
			$valor_banco = publisher_index_normalize_array(json_decode(banco_select_campos_antes($campo_nome), true));
	...
	if($_GESTOR['banco-resultado']){
		...
		// ===== Publisher dropdown
		publisher_index_publisher_options($publisher_id);

		// ===== Template dropdown
		publisher_index_template_options($fields_schema_decoded['template_id'] ?? null, (!empty($html) || !empty($css)));

		// ===== HTML Editor (alvo publisher-index) — edição do template HTML/CSS no banco.
		// A biblioteca html-editor é incluída automaticamente via `bibliotecas` no manifesto.

		$publisher_record = publisher_index_publisher_by_slug($publisher_id);

		// Coletar variáveis @[[item#X]]@ do HTML salvo + variable_mapping para alimentar o html-editor.
		$item_variables = publisher_index_extract_item_variables($html);
...
function publisher_index_clonar(){
	...
	if($_GESTOR['banco-resultado']){
		...
		// HTML Editor (alvo publisher-index) — para que o usuário também possa ajustar
...
function publisher_index_interfaces_padroes(){
	global $_GESTOR;
 ...
 */
function publisher_index_publisher_options($selected_id = null){
	global $_GESTOR;
 ...
 * com `target = 'publisher-index'`. Marca o `id_selecionado` como selected quando informado.
 * Também substitui `#template_placeholder_option#` pelo placeholder padrão de admin-templates
 * (req-004 item 2 e 3).
 */
function publisher_index_template_options($selected_id = null, $has_custom_code = false){
	global $_GESTOR;
...
/**
 * AJAX: dado um template do publisher-index, retorna o nome e os campos `item#X`
 * encontrados no HTML para popular a interface de mapeamento de variáveis.
 */
function publisher_index_ajax_template_load(){
	global $_GESTOR;
	...
	$template = banco_select(Array(
		...
		'extra' => "WHERE id='".banco_escape_field($template_id)."' AND target='publisher-index' AND language='".$_GESTOR['linguagem-codigo']."' AND status='A'"
	));
	...
	$fields = [];
	if($template['html']){
		// req-013 item 5: placeholders sem arrobas — alinhar com publisher-index.widget.php.
 ...
 */
function publisher_index_ajax_publisher_load(){
	global $_GESTOR;

	$publisher_id = $_REQUEST['params']['publisher_id'] ?? '';

	$publisher = publisher_index_publisher_by_slug($publisher_id);
	if(!$publisher){
		$_GESTOR['ajax-json'] = Array(
			'status' => 'Erro',
 ...
 */
function publisher_index_ajax_publisher_pages_search(){
	global $_GESTOR;
 ...
 */
function publisher_index_ajax_publisher_pages_fetch(){
	global $_GESTOR;
 ...
 */
function publisher_index_ajax_widget_preview(){
	global $_GESTOR;
	...
	if(!function_exists('publisher_index_widget_render_inline')){
		require_once(__DIR__.'/publisher-index.widget.php');
	}

	$rendered = publisher_index_widget_render_inline([
		'html' => $html_input,
		'css' => $css_input,
		'publisher_id' => $publisher_id,
...
function publisher_index_start(){
    global $_GESTOR;
	...
	if($_GESTOR['ajax']){
		...
		switch($_GESTOR['ajax-opcao']){
			case 'template-load': publisher_index_ajax_template_load(); break;
			case 'publisher-load': publisher_index_ajax_publisher_load(); break;
			case 'publisher-pages-search': publisher_index_ajax_publisher_pages_search(); break;
			case 'publisher-pages-fetch': publisher_index_ajax_publisher_pages_fetch(); break;
			case 'widget-preview': publisher_index_ajax_widget_preview(); break;
		}

		interface_ajax_finalizar();
	} else {
		publisher_index_interfaces_padroes();

		interface_iniciar();

		switch($_GESTOR['opcao']){
			case 'adicionar': publisher_index_adicionar(); break;
			case 'editar': publisher_index_editar(); break;
			case 'clonar': publisher_index_clonar(); break;
		}
