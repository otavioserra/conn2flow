/**
 * pages-index.widget.js — comportamento público do widget Publicador Índice (req-028 / DEC-041).
 *
 * Incluído no site quando uma página renderiza o widget `pages-index`. Para cada
 * contêiner `.conn2flow-pages-index`, gerencia a interface dinâmica de listagem:
 *   - Busca textual com debounce (300ms) em `.pages-index-search`.
 *   - Ordenação em tempo de execução em `.pages-index-sort`.
 *   - Paginação "Carregar Mais" em `.pages-index-load-more` (append incremental).
 *
 ...
 * (pages_index_render_ajax) devolve `{ status, html, tem_mais }`.
 */
$(document).ready(function () {
    ...
    function initPublisherIndex(el) {
        ...
        function atualizarMetricas(total) {
            ...
            var count = $items.children().not('.pages-index-empty').length;
