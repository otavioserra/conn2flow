...
$(document).ready(function () {
    ...
    var schema = (typeof pages_index_initial_schema !== 'undefined' && pages_index_initial_schema) ? pages_index_initial_schema : {
        rule: 'latest',
        count: 4,
        order_by: 'date_desc',
        selected_items: [],
    ...
    // na tabela pages_index — o valor vive dentro de fields_schema).
    ...
    var $publisher = $('select[name="publisher_id"]'); // req-088: ausente no CRUD (no-op gracioso)
    var $template = $('select[name="template_id"]');

    // req-088: pages-index consulta diretamente a tabela `paginas` — os campos de item são fixos
    // (title/summary/url/date), carregados via AJAX `campos-load` (não dependem de publicador).
    loadCampos();
    ...
    // ===== AJAX: carregar campos fixos disponíveis (req-088)

    function loadCampos() {
        var req = $.extend(true, {}, ajaxDefault, {
            data: $.extend({}, ajaxDefault.data, { ajaxOpcao: 'campos-load' }),
            ajaxOpcao: 'campos-load',
    ...
    // ===== AJAX: carregar publisher (legado — sem uso no pages-index; mantido inerte)
    ...
    function loadTemplate(template_id) {
        var req = $.extend(true, {}, ajaxDefault, {
            ...
            successCallback: function (dados) {
                ...
                // Publicar variáveis do template para o html-editor (alvo pages-index)
                if (typeof window.pages_index_update_target_variables === 'function') {
                    window.pages_index_update_target_variables(availableItemVars);
                }
    ...
    var DEFAULT_PUBLISHER_FIELD_IDS = ['title', 'summary', 'url', 'date'];
    ...
    function syncEditorVariables() {
        if (typeof window.pages_index_update_target_variables !== 'function') return;
        window.pages_index_update_target_variables(availableItemVars);
    }
    ...
    function refreshWidgetPreview(force) {
        ...
        var $iframe = $('#iframe-pages-index-preview');
    ...
    function updateWidgetCodeTab() {
        ...
        var wrapper = '<!-- widgets#pages-index->render({"grupo_slug": "' + slug + '"}) < -->\n'
            + innerHtml + '\n'
            + '<!-- widgets#pages-index->render({"grupo_slug": "' + slug + '"}) > -->';
        ...
        $('#hep-widget-val').val('[[widgets#pages-index->render({"grupo_slug": "' + slug + '"})]]');

        // Incluir o widget no payload de preview para garantir que o editor interno do pages-index receba a variável e possa o AJAX funcionar corretamente.
        widgetsToAjax = widgetsToAjax + (widgetsToAjax.length > 0 ? '<#;>' : '') + 'pages-index->render({\"grupo_slug\": \"' + slug + '\"})';
    }
