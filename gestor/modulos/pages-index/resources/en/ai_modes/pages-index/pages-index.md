...
The whole index MUST be wrapped in a root element with the class `conn2flow-pages-index` and the data attributes below (filled automatically at runtime by global variables). The public widget script uses this class and these attributes for the dynamic interaction:
...
Every HTML snippet representing ONE item (e.g. a card, a list row) MUST be wrapped between the loop markers below, inside a container with the class `pages-index-items`:
...
- Search bar — the input MUST have the class `pages-index-search`:
```
<!-- search-input < -->
<input type="search" class="pages-index-search" placeholder="Search publications...">
<!-- search-input > -->
```

- Sorting — the select MUST have the class `pages-index-sort` and options with the values `date_desc`, `date_asc`, `title_asc`, `title_desc`:
```
<!-- sort-select < -->
...
- "Load more" button — MUST have the class `pages-index-load-more`:
```
<!-- load-more < -->
<button type="button" class="pages-index-load-more">Load more</button>
<!-- load-more > -->
```
...
This page will use the CSS framework `{{framework_css}}`. Use the `<section></section>` tag as the index root container (the root `<section>` is the one receiving the `conn2flow-pages-index` class).
