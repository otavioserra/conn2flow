...
// Ou como include: require_once 'atualizacao-plugin.php'; plugin_process_cli($argv);
// Códigos de saída:
...
function plugin_process_cli(array $params) {
    // Quando incluído no sistema, as bibliotecas já estarão carregadas
    // Só precisamos incluí-las se não estiverem disponíveis
    if (!defined('PLG_EXIT_OK')) {
        require_once __DIR__ . '/../../bibliotecas/plugins-consts.php';
    }
    if (!function_exists('plugin_process')) {
        require_once __DIR__ . '/../../bibliotecas/plugins-installer.php';
    }
    $slug = $params['slug'] ?? null;
    if(!$slug){
        plg_log('-', 'error', 'Slug (id) não informado.');
        return PLG_EXIT_VALIDATE;
    }

    $origemTipo = $params['origem_tipo'] ?? null;
    if(!$origemTipo){
    ...
    plg_log($slug, 'info', 'Início processamento ('.$fase.') origem_tipo='.$origemTipo.(isset($params['reprocessar'])?' reprocessar=1':''));
...
// ===== EXECUÇÃO VIA CLI (apenas quando executado diretamente)
if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'])) {
    // Só inclui as bibliotecas quando executado via CLI
    ...
    $params = [
        ...
        'origem_tipo' => $options['origem_tipo'] ?? null,
