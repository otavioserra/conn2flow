...
function hosts_layouts_adicionar(){
	...
	if(isset($_GESTOR['adicionar-banco'])){
		...
		$retorno = api_cliente_layouts(Array(
			'opcao' => 'adicionar',
			'id' => $id,
		));
...
function hosts_layouts_excluir(){
	global $_GESTOR;
...
function hosts_layouts_interfaces_padroes(){
	global $_GESTOR;
	...
	switch($_GESTOR['opcao']){
		...
		case 'status':
			$_GESTOR['interface'][$_GESTOR['opcao']]['finalizar'] = Array(
				'callbackFunction' => 'hosts_layouts_status',
			);
		break;
		case 'excluir':
			$_GESTOR['interface'][$_GESTOR['opcao']]['finalizar'] = Array(
				'callbackFunction' => 'hosts_layouts_excluir',
			);
		break;
	}
}
...
function hosts_layouts_start(){
	...
	if($_GESTOR['ajax']){
		...
		switch($_GESTOR['ajax-opcao']){
			//case 'opcao': hosts_layouts_ajax_opcao(); break;
		}
		...
		switch($_GESTOR['opcao']){
			case 'adicionar': hosts_layouts_adicionar(); break;
			case 'editar': hosts_layouts_editar(); break;
		}
