    ...
    if($execId) $CONTEXT['exec_id']=$execId;
        ...
        if(!$onlyDb){
            ...
            if(!$dry){
                ...
                // Diagnóstico: capturar hash sentinel antes do wipe/move
                $sentinelRel = 'modulos'.DIRECTORY_SEPARATOR.'admin-arquivos'.DIRECTORY_SEPARATOR.'admin-arquivos.php';
                $sentinelBaseBefore = $BASE_PATH.$sentinelRel;
                $sentinelStagingPath = $realRoot.$sentinelRel;
                $sentinelOldHash = is_file($sentinelBaseBefore)?@hash_file('sha256',$sentinelBaseBefore):null;
                $sentinelNewHash = is_file($sentinelStagingPath)?@hash_file('sha256',$sentinelStagingPath):null;
                $sentinelNewContent = null;
                if(is_file($sentinelStagingPath)) $sentinelNewContent = @file_get_contents($sentinelStagingPath);
                if($sentinelOldHash || $sentinelNewHash){
                    logAtualizacao('Sentinel antes do deploy oldHash='.(string)$sentinelOldHash.' newHash='.(string)$sentinelNewHash,'DEBUG');
                }
                ...
                // Pós-deploy: validar sentinel
                if($sentinelNewHash){
                    $sentinelAfterHash = is_file($BASE_PATH.$sentinelRel)?@hash_file('sha256',$BASE_PATH.$sentinelRel):null;
                    if($sentinelAfterHash !== $sentinelNewHash){
                        logAtualizacao('Sentinel mismatch após deploy (esperado '.$sentinelNewHash.' obteve '.($sentinelAfterHash?:'NULL').') – tentando cópia manual','WARNING');
                        if($sentinelNewContent!==null){
                            $sentPath = $BASE_PATH.$sentinelRel; $sentDir = dirname($sentPath);
                            if(!is_dir($sentDir)) @mkdir($sentDir,0775,true);
                            $w1 = @file_put_contents($sentPath,$sentinelNewContent);
                            if($w1===false){
                                @chmod($sentDir,0775); @chmod($sentPath,0664);
                                $w2 = @file_put_contents($sentPath,$sentinelNewContent);
                                if($w2===false){
                                    $lastErr = error_get_last();
                                    logAtualizacao('Sentinel fallback write falhou: '.($lastErr['message']??'sem detalhe'),'ERROR',true);
                                }
                            }
                            $rehash = is_file($sentPath)?@hash_file('sha256',$sentPath):null;
                            logAtualizacao('Sentinel rehash após fallback='.$rehash,'WARNING');
                        } else {
                            logAtualizacao('Sentinel fallback: conteúdo nulo (não capturado antes do deploy)','ERROR',true);
                        }
                    } else {
                        logAtualizacao('Sentinel validado pós-deploy hash='.$sentinelAfterHash,'DEBUG');
                    }
                }
                // Revalida críticos
...
function webStart(array $req): array {
    ...
    // Instrumentação temporária para diagnosticar file_put_contents null
    $prevHandler = set_error_handler(function($severity,$message,$file,$line){
        if(stripos($message,'file_put_contents')!==false){
            // Loga detalhe completo
            logAtualizacao('Interceptado warning: '.$message.' em '.$file.':'.$line,'ERROR',true);
        }
        return false; // deixa fluxo normal (warning ainda pode ser capturado por output handler)
    });
    $sid = gerarSessionId();
    ...
    if($prevHandler) set_error_handler($prevHandler); // restaura
...
function webDeployFiles(string $sid): array {
    ...
    if(!$dry){
        ...
        // Diagnóstico sentinel (mesma lógica da CLI)
        $sentinelRel = 'modulos'.DIRECTORY_SEPARATOR.'admin-arquivos'.DIRECTORY_SEPARATOR.'admin-arquivos.php';
        $sentinelBaseBefore = $BASE_PATH.$sentinelRel;
        $sentinelStagingPath = $realRoot.$sentinelRel;
        $sentinelOldHash = is_file($sentinelBaseBefore)?@hash_file('sha256',$sentinelBaseBefore):null;
        $sentinelNewHash = is_file($sentinelStagingPath)?@hash_file('sha256',$sentinelStagingPath):null;
        $sentinelNewContent = null; if(is_file($sentinelStagingPath)) $sentinelNewContent = @file_get_contents($sentinelStagingPath);
        if($sentinelOldHash || $sentinelNewHash){ logAtualizacao('WebDeploy sentinel before oldHash='.(string)$sentinelOldHash.' newHash='.(string)$sentinelNewHash,'DEBUG'); }
        $removidos=removerConteudoBase($BASE_PATH,$protegidos);
        $movidos=moverConteudoStaging($realRoot,$BASE_PATH,$protegidos);
        $stats=['removed'=>$removidos,'copied'=>$movidos];
        if($sentinelNewHash){
            $sentinelAfterHash = is_file($BASE_PATH.$sentinelRel)?@hash_file('sha256',$BASE_PATH.$sentinelRel):null;
            if($sentinelAfterHash !== $sentinelNewHash){
                logAtualizacao('WebDeploy sentinel mismatch após deploy (esperado '.$sentinelNewHash.' obteve '.($sentinelAfterHash?:'NULL').') – aplicando fallback','WARNING');
                if($sentinelNewContent!==null){
                    $sentPath=$BASE_PATH.$sentinelRel; $sentDir=dirname($sentPath); if(!is_dir($sentDir)) @mkdir($sentDir,0775,true);
                    $w1=@file_put_contents($sentPath,$sentinelNewContent);
                    if($w1===false){ @chmod($sentDir,0775); @chmod($sentPath,0664); $w2=@file_put_contents($sentPath,$sentinelNewContent); if($w2===false){ $lastErr=error_get_last(); logAtualizacao('WebDeploy sentinel fallback write falhou: '.($lastErr['message']??'sem detalhe'),'ERROR',true); } }
                    $rehash=is_file($sentPath)?@hash_file('sha256',$sentPath):null;
                    logAtualizacao('WebDeploy sentinel rehash após fallback='.$rehash,'WARNING');
                } else { logAtualizacao('WebDeploy sentinel fallback: conteúdo nulo','ERROR',true); }
            } else {
                logAtualizacao('WebDeploy sentinel validado hash='.$sentinelAfterHash,'DEBUG');
            }
        }
    }
