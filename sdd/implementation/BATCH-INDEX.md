...
| BATCH-019 | ready-for-intake | Correções no Menus e Lógica do Módulo de Galerias (req-019) | VALIDATION-CHECKLIST.md#batch-019 | Implementação de target nos links, separador nos templates, comportamento JS dos widgets (dropdowns, mobile hamburguer, carrossel/slider com setas, pontinhos e autoplay) e correção de miniatura de imagem |
| BATCH-020 | blocked | Brainstorm: Autenticação, 2FA, Social Login e Segurança (planejado) | - | Análise de melhorias do módulo perfil-usuario, 2FA, OAuth (Google/Meta), tokens JWT e estrutura de autenticação monolítica |
