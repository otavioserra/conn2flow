...
| BATCH-017 | ready-for-intake | Ajustes e Correções no Módulo de Menus (req-017) | VALIDATION-CHECKLIST.md#batch-017 | Correções no editor HTML (simulação/variáveis), novos componentes de simulação, fix no alternador de tipos JS, melhorias no placeholder do DnD e hover nos submenus |
| BATCH-018 | blocked | Brainstorm: Autenticação, 2FA, Social Login e Segurança (planejado) | - | Análise de melhorias do módulo perfil-usuario, 2FA, OAuth (Google/Meta), tokens JWT e estrutura de autenticação monolítica |
