...
- **Caminho**: [sync-core-to-project.sh](../ai-workspace/en/scripts/projects/sync-core-to-project.sh)
- **Comportamento**:
...
- **Caminho**: [synchronize-project.sh](../ai-workspace/en/scripts/projects/synchronize-project.sh)
- **Modificação**:
...
- **Caminho**: [updates-manager-database.sh](../ai-workspace/en/scripts/dev-environment/updates-manager-database.sh)
- **Modificação**:
  ...
  - **Fallback de dockerPath**: Se `dockerPath` não estiver explícito no bloco do projeto, derivar o caminho dentro do Docker a partir da pasta de testes local (`path_tests` ou `target`) do projeto:
    Substituir a raiz local de sites `<raiz_do_repositorio>/dev-environment/data/sites/` por `/var/www/sites/`.
...
- **Caminho**: [tasks.json](../.vscode/tasks.json)
- **Novas Tasks**:
