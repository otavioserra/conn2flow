# Validation Checklist (Archive: BATCH-001 to BATCH-017)
...
- [x] **Item 5** — Sincronização Sortable.js
...
- [x] Componente de drag-and-drop de árvore bidimensional próprio: Pointer Events (JS vanilla) + visual Fomantic-UI, **sem** jQuery UI/nestedSortable/Sortable.js
...
- [x] Eventos de Pointer para clique expandir/colapsar nós do painel de edição
- [x] Submit de Menus envia o JSON gerado por `buildTree` a partir do modelo flat ordenado

### Renderização e Simulação

- [x] `menus.widget.php` suporta `<!-- item-parent < -->` e `[[item#children]]` para renderização recursiva
- [x] Nós do tipo `pagina` resolvem rótulo/caminho dinamicamente do banco de dados na renderização
- [x] Simulação em `html-editor-interface.js` expande recursivamente a árvore estática mockada com `item-parent`
- [x] Variáveis expostas documentadas e aba Código do Widget atualizada

### Evidência registrada em 2026-06-04

- Validação executável (estática, sem ambiente Docker nesta rodada):
  - `node --check gestor/modulos/menus/menus.js` → `TREE_JS_OK`
  - `node --check gestor/assets/interface/html-editor-interface.js` → `HEI_JS_OK`
  - `php -l gestor/modulos/menus/menus.php` → `No syntax errors detected`
  - `php -l gestor/modulos/menus/menus.widget.php` → `No syntax errors detected`
- Arquivos alterados:
  - `gestor/modulos/menus/menus.php` (removeu Sortable.js e passou a injetar árvore)
  - `gestor/modulos/menus/menus.js` (editor de drag-and-drop Pointer bidimensional, alternador de campos e serialização buildTree)
  - `gestor/modulos/menus/menus.widget.php` (renderização recursiva com DFS, mapeamento do tipo de nó, resolução canônica do banco)
  - `gestor/assets/interface/html-editor-interface.js` (simulação de sub-árvores mockadas)
  - `gestor/modulos/menus/resources/{pt-br,en}/pages/menus-{adicionar,editar,clonar}/*.html` (6 páginas: novos inputs e skeletons)
  - `gestor/modulos/menus/resources/{pt-br,en}/templates/menus-horizontal-navbar/*.html` e demais templates (12 templates com suporte a item-parent e children)
- Pendência (com o operador): rodar `🗃️ Projects - Update => Core` para recompilar templates/páginas e registrar as alterações. Depois, validar manualmente:
  - abrir o CRUD de menus e adicionar um item
  - arrastar e indentar itens criando uma árvore de 3 níveis de profundidade
  - salvar e verificar que reabrir o registro reconstrói a árvore corretamente
  - aba de visualização renderiza os submenus aninhados
  - aba Código do Widget mostra a árvore e as variáveis `[[item#X]]` e `[[item#children]]` no CodeMirror

## BATCH-017 - Ajustes e Correções no Módulo de Menus (req-017)

- [x] **Item 1** — Aba de variáveis e simulação reintegradas no alvo `menus`
  - [x] `html-editor.php` ganha `case 'menus'` carregando controles de simulação
  - [x] Componente `html-editor-menus-simulation` (pt-br/en) criado com árvore de teste de dois níveis de profundidade
  - [x] `html-editor-interface.js` implementa simulação de árvore JS espelhando o DFS do widget
  - [x] Fallback `MENUS_SIM_FALLBACK` injetado localmente se o componente não for carregado do banco
- [x] **Item 2** — Seletor de tipo de item JS lê via Fomantic
  - [x] `menus.js`: alternador lê via `dropdown('get value')` corrigindo a seleção do tipo de item
- [x] **Item 3** — Placeholder do DnD com altura e indicações direcionais
  - [x] `menus.js`: placeholder exibe "Solte o item aqui" / "Drop item here" com setas ← / →
- [x] **Item 4** — Hover de submenus no navbar corrigido via geometria CSS
  - [x] `menus-dropdown` e `menus-horizontal-navbar` (pt-br/en): gaps `mt-1`/`ml-1` removidos, colando submenus
- [x] **Item 5** — Restrição de versionamento respeitada (nenhum `git commit`/`git push`)

### Evidência registrada em 2026-06-04

- Validação estática executada:
  - `node --check gestor/modulos/menus/menus.js` → `JS_OK`
  - `php -l gestor/modulos/menus/menus.php` → `No syntax errors detected`
- Arquivos alterados:
  - `gestor/bibliotecas/html-editor.php` (case 'menus' mapeado + template_map carregado)
  - `gestor/assets/interface/html-editor-interface.js` (simulação robusta de árvore com fallback local)
  - `gestor/modulos/menus/menus.js` (fix dropdown + placeholder customizado)
