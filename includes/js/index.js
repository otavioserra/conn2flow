...
$.alerta_open = function(texto,titulo,botao,callback = false){
	...
	if(!alerta_box_open){
		...
		alerta_box.fadeIn(tempo_animacao,function(){$(this).center();$(this).css('z-index','9999');if(!$('#b2make-alerta-focus-input').length == 0){var focus = $('<input id="b2make-alerta-focus-input" type="text" style="position:absolute;top:-1000px;">'); focus.appendTo('body'); } $('#b2make-alerta-focus-input').focus(); });
		
		alerta_box_open = true;
		
		if(callback){
			alerta_box_callback = callback;
		} else {
			alerta_box_callback = false;
		}
		
		if($('.alert-close').length){
			$('.alert-close').on('mouseup touchend',function(){
				$.alerta_close();
			});
		}
	}
...
$.alerta_close = function(){
	if(alerta_box_callback){
		$('#b2make-listener').trigger(alerta_box_callback);
	}
...
$.alerta_caixa_open = function(p){
	if(!p)p={};
	
	if($('#'+p.id).length > 0){
		if(!alerta_caixa.alerts){
			alerta_caixa.alerts = new Array();
		}
		
		if(!alerta_caixa.alerts[p.id]){
			alerta_caixa.alerts[p.id] = $('<div class="b2make-alerta-caixa" id="b2make-alerta-caixa-'+p.id+'"></div>');
			var position = $('#'+p.id).parent().css('position');
			var pos_change = false;
			
			if(position){
				switch(position){
					case 'static': pos_change = true; break;
				}
			} else {
				pos_change = true;
			}
			...
			if(pos_change){
				$('#'+p.id).parent().css('position','relative');
			}
			
			var left = $('#'+p.id).position().left + $('#'+p.id).outerWidth() + p.left;
			var top = $('#'+p.id).position().top + p.top;
			
			alerta_caixa.alerts[p.id].css({left:left,top:top});
			
			$('#'+p.id).after(alerta_caixa.alerts[p.id]);
			
			$('#'+p.id).on('keyup',function(){
				alerta_caixa.alerts[p.id].hide();
			});
		}
		
		alerta_caixa.alerts[p.id].html(p.mens);
		alerta_caixa.alerts[p.id].show();
	}
}

$.alerta_caixa_close = function(p){
	if(!p)p={};
	
	if(alerta_caixa.alerts){
		alerta_caixa.alerts.forEach(function(item,index,arr){
			item.hide();
		});
	}
}
...
$(document).ready(function(){
	...
	$(document.body).on('click touchstart','#login_box_shadow,#login_box_botao,#login_box_fechar,#esqueceu-senha',function(){
		login_close();
	});
	...
	$(document.body).on('click touchstart','#ajax_janela_shadow,#ajax_janela_fechar',function(){
		janela_close(null);
	});
	...
	$(document.body).on('click touchstart','#ajax_janela_custom_shadow,#ajax_janela_custom_fechar',function(){
		janela_custom_close(null);
	});
	
	$(document.body).on('click touchstart','.ajax_janela_custom',function(){
	...
	$(document.body).on('click touchstart','#alerta_box_shadow,#alerta_box_botao,#alerta_box_fechar',function(){
		$.alerta_close();
	});
	
	$(document.body).on('mouseenter','#alerta_box_fechar',function(){
		$('body').css('cursor', 'pointer');
	});
	
	$(document.body).on('mouseleave','#alerta_box_fechar',function(){
	...
	$.google_recaptcha_reset = function(){
		googleRecaptcha.answered = false;
	}
	
	$.google_recaptcha_is_answered = function(){
		if(googleRecaptcha.answered){
			return true;
		} else {
			return false;
		}
	...
	$.google_recaptcha_answered = function(){
		googleRecaptcha.answered = true;
	}
	
	$.google_recaptcha_response = function(){
		if(grecaptcha){
	...
	$.google_recaptcha_load = function(){
		if(googleRecaptcha.loaded){
			googleRecaptcha.answered = false;
			if(ajax_vars.recaptcha_public_key && grecaptcha)
				grecaptcha.render('recaptcha_div', {
					'sitekey' : ajax_vars.recaptcha_public_key,
					'callback' : 'googleRecaptchaAnswered'
				});
		} else {
			$.getScript('https://www.google.com/recaptcha/api.js?onload=googleRecaptchaCallback&render=explicit',function() {
	...
	function google_recaptcha(){
		...
		window.googleRecaptchaAnswered = function(){
			$.google_recaptcha_answered();
		};
	}
	...
	function aplicar_scripts_iniciais(){
		if($("#_usuario-login-logout").length){
			...
			$(document.body).on('click touchstart',"#_usuario-login-logout-txt",function(){
	...
	function aplicar_scripts(params){
		...
		if($('#esqueceu_senha').length){
			...
			$("#esqueceu_senha").bind('submit',function(){
				...
				campo = "esqueceu_senha-email"; mens = "É obrigatório definir o E-mail!"; if(!$("#"+campo).val()){ $.alerta_caixa_open({id:campo,mens:mens,top:8,left:10}); enviar = false; }
				
				// Checar email
				campo = "esqueceu_senha-email"; mens = "E-mail inválido, preencha o campo de e-mail válido!"; if(!checkMail($("#"+campo).val())){ $.alerta_caixa_open({id:campo,mens:mens,top:8,left:10}); enviar = false; }
		...
		if($('#form_comentarios').length){
			...
			$(document.body).on( 'click' ,'#comentario-cancelar', function(){
	...
	if(variaveis_js.procurar_pesquisa){
		history.replaceState({
			page_url:document.location.protocol+'//'+location.hostname+site_raiz+'procurar/'+variaveis_js.procurar_pesquisa,
	...
	function history_lista(link_call){
		if(!link_call){
			...
			state.page = $(cont_principal).html();
			state.addthis = 0;
			state.menu_paginas = menu_paginas;
			state.menu_pagina = menu_pagina;
			state.opcao_atual = opcao_atual;
			history.replaceState(state,state.page_title,state.page_url);
	...
	window.addEventListener('popstate', function(event) {
		...
		if((/msie|trident/i).test(navigator.userAgent) && link_clicked){
			history_flag = false;
		}
		...
		if(history_flag && !history_first){
			...
			if(state){
				navegacao_conteiner(state,'');
				
				ajax_vars = state.ajax_vars;
				
				if(state.addthis){
					exec_addthis();
				}
				if(state.menu_paginas){
					menu_paginas = state.menu_paginas;
					menu_pagina = state.menu_pagina;
				}
				if(state.audio_path){
					audio_path = state.audio_path;
				}
				
				href_atual = state.page_url;
				document.title = state.page_title;
				
				menu_principal();
				
				opcao_atual = state.opcao_atual;
				...
				aplicar_scripts_after({href:state.page_url,banner_href:state.page_url,history:true});
				aplicar_scripts({href:state.page_url,janela:state.janela,history:true});
			}
		}
	...
	$(document.body).on('click','a,area',function(event){
		history_flag = false;
		flags['scroll'] = false;
		var href = this.href;
						...
						$('<div id="_cont_ajax"></div>').load(href,{ajax_page:1,projeto_js:(projeto_js?JSON.stringify(projeto_js):{})}, function(response, status, xhr) {
							...
							if(status == "error"){
								...
								if(
									...
									if(dados.b2make_loja_iframe_redirect){
										window.parent.postMessage({opcao:'redirect',url:dados.b2make_loja_url_atual}, '*');
										return false;
									}
	...
	function exec_addthis(){
		if(location.hostname != 'localhost'){
			if(window.addthis){
				...
				window.addthis.update('config', 'data_track_clickback', true);
				window.addthis.update('share', 'url', state.page_url);
				window.addthis.update('share', 'title', state.page_title);
				window.addthis.toolbox(".addthis_toolbox");
			}
		}
	}
	...
	$(document.body).on('mouseenter',".link_hover",function(){
		$('body').css('cursor', 'pointer');
	});
	
	$(document.body).on('mouseleave',".link_hover",function(){
		$('body').css('cursor', 'default');
	});
	
	$(document.body).on('mouseenter',".image_hover",function(){
	...
	$(document.body).on('mouseleave',".image_hover",function(){
		var src = $(this).attr("src");
		var url = $(this).attr("url");
		$(this).attr("src",url);
		$(this).attr("url",src);
	...
	$(document.body).on('mouseup',"a > .image_hover",function(){
		var src = $(this).attr("src");
		var url = $(this).attr("url");
		$(this).attr("src",url);
		$(this).attr("url",src);
	...
	$(document.body).on('mousedown',"a > .image_hover",function(){
		
	});
	
	$(document.body).on('click touchstart',".image_hover",function(){
		var src = $(this).attr("src");
		var url = $(this).attr("url");
		$(this).attr("src",url);
		$(this).attr("url",src);
	...
	$(document.body).on('mouseenter',".imagens_animate div.boxgrid",function(){
	...
	$(document.body).on('mouseleave',".imagens_animate div.boxgrid",function(){
	...
	$(document.body).on('submit',"#login_box",function() {
		var enviar = true;
		var campo;
		var mens;
	...
	$(document.body).on('click touchstart',"#contato_botao",function(){
		var enviar = true;
		var campo;
		var post;
		var mens;
	...
	$(document.body).on('keyup',"#buscar_pesquisa",function(e){
		if(e.which == 13){
			pesquisar();
		}
	});
	
	$(document.body).on('click touchstart',"#buscar_botao",pesquisar);
	
	$(document.body).on('click touchstart',"#emarkenting_botao",function(){
		var enviar = true;
		var campo;
		var post;
		var mens;
	...
	$(document.body).on('submit',"#form_pesquisar",function(){
		return false;
	});
	
	$(document.body).on('click touchstart',".enviar_formulario",function(){
	...
	window.enviar_form_simples = function enviar_form_simples(form_id){
		...
		if(cms.enviar_form_simples)if(cms.enviar_form_simples.no_reset)no_reset = true;
		
		if(!no_reset)$('#'+form_id)[0].reset();
		...
		$.ajax({
			...
			success: function(txt){
				...
				if(
					...
					if(dados.ajax_callback) $(dados.ajax_callback).trigger((dados.ajax_callback_event ? dados.ajax_callback_event : 'execute'));
					
					if(!cms.nao_parar_lendo)$('#ajax_lendo').fadeOut(tempo_animacao);
			...
			success: function(txt){
				...
				if(
					...
					if(mudar_pagina){
						...
						history.pushState({
							page_url:document.location.protocol+'//'+location.hostname+site_raiz+href+(dados.procurar_pesquisa?'/'+dados.procurar_pesquisa:''),
							page_title:dados.titulo,
							page:dados.page,
							ajax_vars:ajax_vars,
							addthis:dados.addthis,
					...
					console.log('ERROR - enviar_form - '+txt);
				}
			},
			error: function(txt){
				$('#ajax_erro').fadeIn(tempo_animacao);
				setTimeout(function(){ $('#ajax_erro').fadeOut(tempo_animacao); }, 1000);
				console.log('ERROR - enviar_form - '.dataString);
			}
		});
	}
	...
	$.b2make_check_box_load = function(){
	...
	$(document.body).on('mouseup tap','.b2make-check-box div',function(e){
