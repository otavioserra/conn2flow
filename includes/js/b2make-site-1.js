...
$(document).ready(function () {
	...
	function services_quantidade_change_start(p) {
		b2make.services_list = p.json;
		services_carrinho_mudar_quantidade(p.value);
	}
		...
		if (!b2make.services_list) {
			if (b2make.site_cache.ativo) {
				$('body').on('site-cache-ready-services', function () {
					services_quantidade_change_start({ json: b2make_site_cache_get({ name: 'services' }), value: value });
				});
				...
				$.ajax({
					...
					success: function (txt) {
						services_quantidade_change_start({ json: txt, value: value });
				...
				if (b2make.services_list[i].id == id) {
					...
					if (quantidade > 0) {
						if (quantidade >= value) {
							...
							cont.find('.b2make-sclt-quant').find('input').attr('value', '1');
						}
					} else {
						$.dialogbox_open({
	...
	function services_carrinho_add(p) {
		...
		var mobile = b2make.mobile.active;
		...
		if (!found) {
			...
			var nome = $('<div class="b2make-services-carrinho-lista b2make-sclt-nome b2make-sclt-nome-ajuste"' + (mobile ? ' data-mobile="true"' : '') + '>' + escapeHtml(p.nome) + '</div>');
			...
			var preco_unitario = $('<div class="b2make-services-carrinho-lista b2make-sclt-preco-unitario"' + (mobile ? ' data-mobile="true"' : '') + '>R$ ' + services_format_money(p.preco, 2, ',', '.') + '</div>');
			var sub_total = $('<div class="b2make-services-carrinho-lista b2make-sclt-sub-total"' + (mobile ? ' data-mobile="true"' : '') + '>R$ ' + services_format_money(p.preco, 2, ',', '.') + '</div>');
	...
	function services_carrinho_finalizar_start(p) {
		var pub_id = p.json.pub_id;
		var data = '';
		...
		window.open('https://' + (alpha ? 'alpha.' : '') + (beta ? 'beta.' : '') + 'b2make.com/' + (debug ? 'teste/' : '') + 'e-services/pre-checkout/' + pub_id + '/' + data, '_self');
	}

	function services_carrinho_continuar_start(p) {
		var url_continuar_comprando = p.json.url_continuar_comprando;
	...
	function services_carrinho_abrir() {
		if (!b2make.services_carrinho_open) {
			...
			if (!b2make.services_carrinho_cont) {
				...
				carrinho = $('<div id="b2make-services-carrinho"' + (mobile ? ' data-mobile="true"' : '') + '></div>');
				...
				var lista_tit_nome = $('<div class="b2make-services-carrinho-lista-tit b2make-sclt-nome"' + (mobile ? ' data-mobile="true"' : '') + '>Servi&ccedil;os</div>');
				...
				var lista_tit_preco_unitario = $('<div class="b2make-services-carrinho-lista-tit b2make-sclt-preco-unitario"' + (mobile ? ' data-mobile="true"' : '') + '>Pre&ccedil;o unit.</div>');
				var lista_tit_sub_total = $('<div class="b2make-services-carrinho-lista-tit b2make-sclt-sub-total"' + (mobile ? ' data-mobile="true"' : '') + '>Sub Total</div>');
				...
				if (mobile) {
					carrinho_lista.find('.b2make-sclt-nome').attr('data-mobile', 'true');
					carrinho_lista.find('.b2make-sclt-preco-unitario').attr('data-mobile', 'true');
					carrinho_lista.find('.b2make-sclt-sub-total').attr('data-mobile', 'true');
				} else {
					carrinho_lista.find('.b2make-sclt-nome').removeAttr('data-mobile');
					carrinho_lista.find('.b2make-sclt-preco-unitario').removeAttr('data-mobile');
					carrinho_lista.find('.b2make-sclt-sub-total').removeAttr('data-mobile');
				}
				...
				$('#b2make-services-carrinho-finalizar').on('mouseup tap', function (e) {
					...
					if (b2make.site_cache.ativo) {
						$('body').on('site-cache-ready-services-config', function () {
							services_carrinho_finalizar_start({ json: b2make_site_cache_get({ name: 'services-config' }) });
						});
						...
						$.ajax({
							...
							success: function (txt) {
								services_carrinho_finalizar_start({ json: txt });
							},
							error: function (txt) {
								console.log('ERROR AJAX - ' + id_func + ' - ' + txt);
							}
				...
				$('#b2make-services-carrinho-continuar').on('mouseup tap', function (e) {
					...
					if (b2make.site_cache.ativo) {
						$('body').on('site-cache-ready-services-config', function () {
							services_carrinho_continuar_start({ json: b2make_site_cache_get({ name: 'services-config' }) });
						});
						...
						$.ajax({
							...
							success: function (txt) {
								services_carrinho_continuar_start({ json: txt });
							},
							error: function (txt) {
								console.log('ERROR AJAX - ' + id_func + ' - ' + txt);
							}
	...
	function services() {
		...
		$('.b2make-service-inserir-carrinho-2').on('mouseup tap', function (e) {
			if (e.which != 1 && e.which != 0 && e.which != undefined) return false;
	...
	function posts_filter_widget_menu_html(p) {
		...
		if ($(p.obj).attr('data-layout-tipo') != 'menu') {
			var options = localStorage.getItem('b2make.posts-filter-options');
	...
	function widget_loja_start(p) {
		...
		if (widget_loja) {
			var alpha = getUrlParameter('alpha');
			var beta = getUrlParameter('beta');
			var url = 'https://' + (alpha ? 'alpha.' : '') + (beta ? 'beta.' : '') + 'b2make.com/e-services/' + loja_id + '/';

			var widget_loja_cont = $('<div id="b2make-widget-loja"></div>');

			var dados_pessoais = $('<a id="b2make-widget-loja-dados-pessoais" href="' + url + 'my-account"></a>');
			var carrinho = $('<a id="b2make-widget-loja-carrinho" href="' + url + 'cart"></a>');
