window.aplicar_scripts_ecommerce = function(params){
		...
		if($("#cont-"+mais_resultados.cel+"-mais").length){
			...
			$("#cont-"+mais_resultados.cel+"-mais").bind('click touchstart',function(){
				...
				$.ajax({
					...
					data: { ajax : 'sim' , opcao : ajax_dados.opcao, limite : ajax_dados.limite, categorias_produtos : ajax_dados.categorias_produtos, page : mais_resultados.page },
					beforeSend: function(){
						$('#ajax_lendo').fadeIn(tempo_animacao);
					},
					success: function(txt){
	...
	if($('#_meus-pedidos-cont').length){
		...
		$("._meus-pedidos-voucher").live('click touchstart',function(){
			var id = $(this).attr('data-id');
		...
		$("._meus-pedidos-pagar").live('click touchstart',function(){
			var id = $(this).attr('data-id');
