...
$(document).ready(function(){
	...
	var data_id = variaveis_js.eventos_data_id;
	
	$("#datas-add").bind("click touchstart", function() {
		var datas_padrao = $('#datas-padrao').html();
		data_id++;
		
		var ids_data = $('#datas-ids-html').val();
	
		if(ids_data.length > 0){
			$('#datas-ids-html').val(ids_data+','+data_id);
		} else {
			$('#datas-ids-html').val(data_id);
		}
		
		datas_padrao = datas_padrao.replace(/#id-data#/gi,data_id);
		datas_padrao = datas_padrao.replace(/#data_val#/gi,'');
		datas_padrao = datas_padrao.replace(/#hora_val#/gi,'');
		
		$('#datas-cont').append(datas_padrao);
	...
	$(".excluir-data").live("click touchstart", function(event) {
		var id = this.id;
		id = id.replace(/data-/gi,'');
		...
		for(var i=0;i<ids_arr.length;i++){
			if(ids_arr[i] != id){
				ids_aux = ids_aux + (ids_aux.length > 0 ? ',' : '') + ids_arr[i];
			}
		}
		
		$('#datas-ids-html').val(ids_aux);
	});
	...
	$(".tags-checkbox").live('change',function(e){
		$("#tags-flag").val('s');
	});
	
	$(".tags-nome").live('click touchstart',function(e){
		if(e.target.nodeName != 'INPUT'){
	...
	$(".tags-excluir").live('click touchstart',function(){
	...
	$("#galeria_escolher").bind("click touchstart", function() {
		...
		popup.dialog('option','title','Esolha a Galeria de Imagens');
		popup.dialog('open');
		popup_ativo = true;
	});
	...
	$("#videos_escolher").bind("click touchstart", function() {
		...
		popup.dialog('option','title','Esolha a Galeria de VÌdeos');
		popup.dialog('open');
		popup_ativo = true;
	});
