...
function pagina_trocar_variavel_valor($variavel,$valor,$variavelEspecifica = false,$celula = false){
	/**********
		Descrição: trocar variável da página por um valor.
	**********/
	...
	if(isset($variavel)){
		if($celula){
			...
			if($variavelEspecifica){
				$_GESTOR['pagina'] = modelo_var_troca_tudo($_GESTOR['pagina'],$variavel,(isset($valor) ? $valor : ''));
			} else {
				$open = $_GESTOR['variavel-global']['open'];
				$close = $_GESTOR['variavel-global']['close'];
				
				$_GESTOR['pagina'] = modelo_var_troca_tudo($_GESTOR['pagina'],$open.$variavel.$close,(isset($valor) ? $valor : ''));
			}
		}
	}
}
...
function pagina_trocar_variavel($params = false){
	...
	**********/
	...
	if(isset($codigo) && isset($variavel)){
		$open = $_GESTOR['variavel-global']['open'];
		$close = $_GESTOR['variavel-global']['close'];
		
		return modelo_var_troca_tudo($codigo,$open.$variavel.$close,(isset($valor) ? $valor : ''));
	}
}
