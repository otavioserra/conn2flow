...
function connect_b2make($_params = false){
	global $_B2MAKE;
	global $_LOCAL_ID;
	...
	// interface - STRING - Local onde será acessado as informações
	// ============== Parâmetros opcionais
	// params - ARRAY - Parâmetros que serão enviados para a interface
	...
	if(!$obj_xml){
		return Array(
			'error' => 'XML retornado inválido. Retorno bruto: '.$xml
		);
	}
	
	if(count($obj_xml->error) > 0){
		return Array(
			'error' => 'Erro: '.$obj_xml->error
		);
	}
